...
public class RemotiTunesMain {
		...
		executor.execute(new Runnable() {
			...
			public void run() {
				if (connection == null &&
					...
					RemotiTunesConfiguration config = frame.getCurrentConfiguration(true);
					try {
	...
	private void setupRendezvous(RemotiTunesConfiguration config)
			throws IOException {
	...
	private void initGUI() {
		...
		menu = new JPopupMenu();
		connectMenuItem = menu.add(new ConnectAction("接続する"));
		disconnectMenuItem = menu.add(new DisconnectAction("切断する"));
		menu.addSeparator();
	...
	private class RemotiShutdownHook extends Thread {
		...
		@Override
		public void run() {
			RemotiTunesConfiguration config = frame.getCurrentConfiguration(false);
